...
def plotTree(myTree, parentPt, nodeTxt):#if the first key tells you what feat was split on
    ...
    plotNode(firstStr, cntrPt, parentPt, decisionNode)  # 判断节点，cntrPt为其坐标，判断节点要位于其下叶节点的中央
    secondDict = myTree[firstStr]
    ...
    for key in secondDict.keys():
        if type(secondDict[key]).__name__=='dict':#test to see if the nodes are dictonaires, if not they are leaf nodes
            ...
            plotNode(secondDict[key], (plotTree.xOff, plotTree.yOff), cntrPt, leafNode) # 叶节点，（xOff，yOff）为其坐标
...
def createPlot(inTree):
    ...
    fig.clf()   #清空绘图区域
    axprops = dict(xticks=[], yticks=[])    #创建一个轴属性的字典，去掉坐标轴，下面作为sbuplot函数的参数
    createPlot.ax1 = plt.subplot(111, frameon=False, **axprops)     #叠加图层时frameon必须设置成False，不然会覆盖下面的图层
